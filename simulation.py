    ...
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        if direction == 'right':
            if len(vehicles[direction][lane]) > 1 and vehicles[direction][lane][self.index - 1].crossed == 0:
                # if more than 1 vehicle in the lane of vehicle before it has crossed
                # stop line
        ...
        if self.direction == 'right':
            ...
            if self.willTurn == 1:
                if self.crossed == 0 or self.x + self.currentImage.get_rect().width < mid[self.direction]['x']:
                    ...
                    if self.turned == 0:
                        ...
                        if self.rotateAngle == 90:
                            ...
                            # self.vehicleClass + ".png" self.x = mid[self.direction]['x'] self.y = mid[
                            # self.direction]['y'] self.image = pygame.image.load(path)
                    else:
                ...
                if ((self.x + self.currentImage.get_rect().width <= self.stop or self.crossed == 1 or (
                    ...
                    # and (it is either the first vehicle in that lane or it is has enough gap to the next vehicle in
                    # that lane)
            ...
            if self.willTurn == 1:
                if self.crossed == 0 or self.x > mid[self.direction]['x']:
                    ...
                    if self.turned == 0:
                        ...
                        if self.rotateAngle == 90:
                            ...
                            # path = "images/" + directionNumbers[((self.direction_number+1)%noOfSignals)] + "/" +
                            # self.vehicleClass + ".png" self.x = mid[self.direction]['x'] self.y = mid[
                            # self.direction]['y'] self.currentImage = pygame.image.load(path)
                    else:
                ...
                if ((self.x >= self.stop or self.crossed == 1 or (currentGreen == 2 and currentYellow == 0)) and (
                    ...
                    self.x -= self.speed  # move the vehicle    
            # if((self.x>=self.stop or self.crossed == 1 or (currentGreen==2 and currentYellow==0)) and (
            # self.index==0 or self.x>(vehicles[self.direction][self.lane][self.index-1].x + vehicles[
            # self.direction][self.lane][self.index-1].currentImage.get_rect().width + gap2))): self.x -= self.speed
...
def setTime():
    ...
    # detection_result=detection(currentGreen,tfnet) greenTime = math.ceil(((noOfCars*carTime) + (
    # noOfRickshaws*rickshawTime) + (noOfBuses*busTime) + (noOfBikes*bikeTime))/(noOfLanes+1)) if(
    # greenTime<defaultMinimum): greenTime = defaultMinimum elif(greenTime>defaultMaximum): greenTime =
    # defaultMaximum greenTime = len(vehicles[currentGreen][0])+len(vehicles[currentGreen][1])+len(vehicles[
    # currentGreen][2]) noOfVehicles = len(vehicles[directionNumbers[nextGreen]][1])+len(vehicles[directionNumbers[
    # nextGreen]][2])-vehicles[directionNumbers[nextGreen]]['crossed'] print("no. of vehicles = ",noOfVehicles)
